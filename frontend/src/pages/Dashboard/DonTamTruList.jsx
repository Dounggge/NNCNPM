...
export default function DonTamTruList() {
  const navigate = useNavigate();
  const { user, canAccess } = useAuth();
  const [loading, setLoading] = useState(true);
  const [dons, setDons] = useState([]);
  const [filter, setFilter] = useState('cho_xu_ly'); // cho_xu_ly, da_xu_ly, all
  ...
  const fetchDons = async () => {
    try {
      ...
      const response = await donTamTruAPI.getAll(params);
  ...
  const handleApprove = async (id) => {
    if (!window.confirm('✅ Xác nhận DUYỆT đơn tạm trú này?')) return;
  ...
  return (
    <>
      <PageMeta title="Danh sách đơn Tạm trú" />
      <PageBreadcrumb
        pageTitle="Danh sách đơn Tạm trú"
        items={[
          ...
          <div>
            ...
            <p className="mt-2 text-gray-600 dark:text-gray-400">
              {canAccess(['admin', 'to_truong']) 
                ? 'Quản lý và duyệt đơn tạm trú' 
                : 'Danh sách đơn tạm trú của bạn'}
            </p>
          </div>
              ...
              <table className="w-full">
                <thead className="bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Người tạm trú</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Địa chỉ tạm trú</th>
                    ...
                    <tr key={don._id} className="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                      ...
                      <td className="px-4 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex justify-end gap-2">
                          {/* CHI TIẾT */}
                          <Link
                            to={`/dashboard/don-tam-tru/${don._id}`}
